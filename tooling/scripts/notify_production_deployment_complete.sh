...
function validate_env_vars() {
  ...
  if [ "$CONFLUENCE_RELEASE_PAGE_URL" == "" ]; then
    echo "CONFLUENCE_RELEASE_PAGE_URL is required"
    exit 1
  fi
}
...
function notify_slack_deployment_message() {
  ...
  local message="🎉 *${repo}-${version} has been deployed to production* 🎉\n• workflow: ${workflow_run_url}\n• Jira tickets: https://${JIRA_DOMAIN:-your-jira-domain.atlassian.net}/issues/?jql=fixVersion%20%3D%20${repo}-v${version}\n• Release Notes: ${CONFLUENCE_RELEASE_PAGE_URL}\n• Github Release: https://github.com/${GITHUB_REPOSITORY_OWNER:-YOUR_ORG}/${repo}/releases/tag/v${version}"
  echo "$message"
}
